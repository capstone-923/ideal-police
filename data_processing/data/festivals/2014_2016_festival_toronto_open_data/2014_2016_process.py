...
def processCSV(input_file: str) -> pd.DataFrame:
    """
    Processes the input CSV file by merging columns and removing unwanted columns.

    Args:
        input_file (str): The path to the CSV file to be processed.
...
def dateExpansion(input_df: pd.DataFrame) -> pd.DataFrame:
    """
    Expands the date columns in the input DataFrame by splitting the start and end dates 
    into day, month, and year columns.

    Args:
        input_df (pd.DataFrame): The DataFrame containing the start and end date columns.

    Returns:
        pd.DataFrame: The updated DataFrame with expanded date columns.
    """
...
def fullDates(input_df: pd.DataFrame):
    """
    Expands the date range between start and end dates in the input DataFrame. 
    For each event, a new row is created for each day it spans, and 
    the corresponding day, month, and year are added as separate columns.
    Creates a CSV file in the end. 

    Args:
        input_df (pd.DataFrame): The DataFrame containing start and end dates (start_year, start_month, start_day, end_year, end_month, end_day).
